...
namespace FalcaPOS.Stock.ViewModels
{
    public class StockReceiverViewModel : BindableBase
    {
        ...
        public DelegateCommand<object> EnableDisableTransportPayerCommand { get; private set; }
        ...
        public StockReceiverViewModel(ProgressService ProgressService, IStockTransferService StockTransferService, EventAggregator EventAggregator, INotificationService NotificationService, Logger Logger)
        {
            ...
            IsTransportChargesBearerEnabled = false;

            EnableDisableTransportPayerCommand = new DelegateCommand<object>(EnableDisableTransportPayer);


            //eventAggregator.GetEvent<ReloadStockReceiverListForSahayaEvent>().Subscribe(GetStockReceiverList);

        }

        private void EnableDisableTransportPayer(object obj)
        {
            if (UpdateReceivedProduct.TransportCharges > 0 || UpdateReceivedProduct.Others > 0)
            {
                IsTransportChargesBearerEnabled = true;
            }
            else
            {
                IsTransportChargesBearerEnabled = false;
        ...
        public void StockReceiverUpdate(object param)
        {
            try
            {
                if (UpdateReceivedProduct.TransportCharges > 0 || UpdateReceivedProduct.Others > 0)
                {
                    if (string.IsNullOrEmpty(UpdateReceivedProduct.TransportChargesPayer))
                    {
                        notificationService.ShowMessage("Please select transport charge payer", NotificationType.Error);
                        return;
                    }
                }
            ...
            try
            {
                ...
                if (stockReceiverProduct != null)
                {
                    if (stockReceiverProduct.StockTransferList != null && stockReceiverProduct.StockTransferList.Count > 0)
                    {
                        ...
                        UpdateReceivedProduct.ToLocation = AppConstants.LoggedInStoreInfo.Name;
                        UpdateReceivedProduct.TransportChargesPayers = new List<string> { UpdateReceivedProduct.FromLocation, UpdateReceivedProduct.ToLocation };
            ...
            try
            {
                ...
                if (_viewmodel != null)
                {
                    ...
                    ReceiverUpdateModel receiverUpdateModel = new ReceiverUpdateModel()
                    {
                        ...
                        TransportChargesPayer = (_viewmodel.UpdateReceivedProduct.Others > 0  ||  _viewmodel.UpdateReceivedProduct.TransportCharges >0 )?  _viewmodel.UpdateReceivedProduct.TransportChargesPayer : string.Empty,
        ...
        public void DownloadEwayBill(object obj)
        {
            try
            {
                var _model = (ReceiverViewModel)obj;
                if (_model != null && _model.EwayBill != null)
                {
                    if (_model.EwayBill != null)
                    {
                        ProcessStartInfo _p = new ProcessStartInfo
                        {
                            UseShellExecute = true,
                            FileName = "http://" + _model.EwayBill,
                        };
        ...
        private bool _isTransportChargesBearerEnabled;

        public bool IsTransportChargesBearerEnabled
        {
            get { return _isTransportChargesBearerEnabled; }
            set { SetProperty(ref _isTransportChargesBearerEnabled, value); }
        }
