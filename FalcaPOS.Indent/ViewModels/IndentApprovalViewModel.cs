...
namespace FalcaPOS.Indent.ViewModels
{
    public class IndentApprovalViewModel : BindableBase {
        ...
        public DelegateCommand<object> PaymentBackCommand { get; private set; }
        public DelegateCommand<object> GstChangeCommand { get; private set; }
        public DelegateCommand AddPaymnetCommand { get; private set; }
        public DelegateCommand DiscountBackCommand { get; private set; }
        public DelegateCommand<object> AddDiscountCardCommand { get; private set; }
        public DelegateCommand<object> RemoveDiscountCardCommand { get; private set; }
        public DelegateCommand<object> DiscountValueChangeCommand  { get; private set; }
        public DelegateCommand<object> DiscountPercentageChangeCommand { get; private set; }
        ...
        public IndentApprovalViewModel(IUnityContainer container, IDialogService dialogService, ISupplierService supplierService, Logger logger, IEventAggregator eventAggregator, IIndentService indentService, INotificationService notificationService, ProgressService progressService, ICommonService commonService, IInvoiceFileService invoiceFileService)
        {
            ...
            StepsInfo = new ObservableCollection<StepInfo>()
            {
                new StepInfo() { Name = "Add supplier" },
                new StepInfo() { Name = "Add Product Price"},
                new StepInfo() { Name = "Add Payment"},
                 new StepInfo() { Name ="Terms & Conditions"},
               
            };

            SelectedIndex = 0;

            PaymentBackCommand = new DelegateCommand<object>(PayBackCommand);

            AddSupplierToIndent.IsTDSApplicable = false;

            GstChangeCommand = new DelegateCommand<object>(GSTChange);

            AddPaymnetCommand = new DelegateCommand(AddPaymentContinue);

            DiscountBackCommand = new DelegateCommand(DiscountBack);

            AddDiscountCardCommand = new DelegateCommand<object>(AddNewDiscountField);

            RemoveDiscountCardCommand = new DelegateCommand<object>(RemoveDiscountfield);

            DiscountValueChangeCommand = new DelegateCommand<object>(DiscountValueCal);

            DiscountPercentageChangeCommand = new DelegateCommand<object>(DiscountPercentageCal);

            TransportChargesPayers = ApplicationSettings.TRANSPORT_CHARGES_PAYER.ToList();
        }
        ...
        public void AddSupplier(object obj)
        {
            try
            {
                ...
                if (DiscountDetails != null)
                {
                    if (DiscountDetails.Count > 1)
                    {
                        ...
                        {    
                            ...
                            {
                                _notificationService.ShowMessage("Please enter Name at Row "+i, NotificationType.Error);
                                return;
                            }
                            if (item.DisocuntValue==0 && item.DisocuntPercentage==0 )
                            {
                                _notificationService.ShowMessage("Please enter Flat or Percentage at Row "+i, NotificationType.Error);
                                return;
                            }
                            i++;
                        }
                ...
                if (AddSupplierToIndent.TransportCharges > 0)
                {
                    if(string.IsNullOrEmpty(TransportChargesPayer))
                    {
                        _notificationService.ShowMessage("Please select transport payer", NotificationType.Error);
                        return;
                    }
                }

                var FirstRow = DiscountDetails.FirstOrDefault();
                
                if(FirstRow.DisocuntValue>0 || FirstRow.DisocuntPercentage > 0)
                {
                    if(string.IsNullOrEmpty(FirstRow.Name))
                    {
                        _notificationService.ShowMessage("Please enter Name",NotificationType.Error);
                        return;
                ...
                if (!string.IsNullOrEmpty(FirstRow.Name))
                {
                    if (FirstRow.DisocuntValue==0 && FirstRow.DisocuntPercentage==0)
                    {
                        _notificationService.ShowMessage("Please enter flat or percentage", NotificationType.Error);
                        return;

                    }
                }
                var _discountAmount = DiscountDetails.Sum(x => x.DisocuntValue);
                if(AddSupplierToIndent.PayableAmount<=_discountAmount)
                {
                    _notificationService.ShowMessage("Flat discount should be less than po amount", NotificationType.Error);
                    return;
                }
            ...
            try
            {
                ...
                if (model != null)
                {
                    ...
                    else if (model.Status.ToLower() == IndentStatus.approve.ToString())
                    {
                        ...
                        SelectedIndex = 0;
                        AddSupplierToIndent.IsTDSApplicable = false;
                        AddSupplierToIndent.SelectedSupplier = null;
                        AddSupplierToIndent.PartialPayment.TdsAmount = 0;
                        TransportChargesPayer = null;
                    ...
                    else if (model.Status.ToLower() == IndentStatus.intransit.ToString())
                    {
                        if (model.IndentInventoryDetails != null && model.IndentInventoryDetails.Select(x => x.InvoiceNo).FirstOrDefault() != AppConstants.YETTOADD)
                        {
                            ...
                            GSTType = (model.Products.FirstOrDefault()?.SelectedGSTslab?.GstType != null && model.Products.FirstOrDefault()?.SelectedGSTslab?.GstType== "Exclusive") ? "Exclusive GST" : "Inclusive GST";
                    ...
                    else if (model.Status.ToLower() == IndentStatus.received.ToString() && model.PayableAmount != model.PaymentsList.Where(x => x.PaidDate != null && x.PayrefNo != null).Sum(x => x.PaymentTotal+x.TdsAmount))
                    {
                    ...
                    else if (model.Status.ToLower() == IndentStatus.fullpaid.ToString() || model.IsFullPaid == true)
                    {
                        ...
                        if (model.PayableAmount != model.PaymentsList.Where(x => x.PaidDate != null && x.PayrefNo != null).Sum(x => x.PaymentTotal+x.TdsAmount))
                        {
                ...
                if (indentView != null)
                {
                    ...
                    foreach (var item in indentView.AcivityLogs)
                    {
                        ...
                        activityLog.Add(new ActivityLogs()
                        {
                            ...
                            Date = item.Date,
                            Status = (item.Status.ToLower() == IndentStatus.created.ToString()) ? ProperStatus.ProperStatusName(item.Status) : (""==_previousStatus) ? ProperStatus.ProperStatusName(item.Status): ProperStatus.ProperStatusName(_previousStatus) + " ----- " + ProperStatus.ProperStatusName(item.Status),
                    ...
                    {
                        if (item.SelectedGSTslab?.GstType == "Exclusive")
                        {
                            totalEstimatedAmount +=(float)((item.EstimatedPrice*item.AvailableQty) +(item.EstimatedPrice /100)*item.SelectedGSTslab?.GstValue * item.AvailableQty);
                        }
                        else
                        {
                            totalEstimatedAmount += item.EstimatedPrice * item.AvailableQty;
                        }
                    }
                    ProcessindentView.TotalEstimantedAmount = (float)Math.Round(totalEstimatedAmount, MidpointRounding.AwayFromZero);
                    ...
                    ProcessindentView.IsInclusiveGST = indentView.SupplierName != null ? ((indentView.Products != null && indentView.Products.ToList().Count > 0) ? (indentView.Products.Select(x => x.SelectedGSTslab?.GstType).FirstOrDefault() != null && (indentView.Products.Select(x => x.SelectedGSTslab?.GstType).FirstOrDefault()== "Exclusive")? false : true) : false) : false;
                    ...
                    ProcessindentView.PayableAmount = ((float)Math.Round(totalAmount, 0, MidpointRounding.AwayFromZero));
                    //var resultTds = (indentView.PaymentsList != null && indentView.PaymentsList.Count > 0) ? indentView.PaymentsList.Take(indentView.PaymentsList.Count() - 1).ToList().Sum(x=>x.TdsAmount) : 0;
                    var paid = (indentView.PaymentsList != null && indentView.PaymentsList.Count > 0) ? (indentView.PaymentsList.Sum(x => x.PaymentTotal+x.TdsAmount)): 0;
                    ...
                    // check if payable amount is not equal to sum of (payment total + tds amount) in the payments list.
                    indentView.IsFullPaid = (ProcessindentView.PayableAmount != (indentView.PaymentsList.Sum(x => x.PaymentTotal + x.TdsAmount)))? false : true;
                    ...
                    var newbal = ((ProcessindentView.PayableAmount + (indentView.PaymentsList.Sum(x => x.TdsAmount))) < (indentView.PaymentsList.Sum(x => x.PaymentTotal))) ? true: false ;

                    // Set the visibility of the '+Add Payment' to false when:
                    // 1. logged in role is not PM
                    // 2. If logged in role is PM and Payment type of Indent is FullPaid
                    // 3. If logged in role is PM and payment type of Indent is partial and balance payment is 0
                    ProcessindentView.IsUpateBtnVisiblity = (AppConstants.ROLE_PURCHASE_MANAGER == AppConstants.USER_ROLES[0].ToString()) ? indentView.IsFullPaid ? false: (balance == 0) ? false : true : false;

                    //ProcessindentView.IsUpateBtnVisiblity = (AppConstants.ROLE_PURCHASE_MANAGER == AppConstants.USER_ROLES[0].ToString()) ? newbal ? false : true : true;
        ...
        /// <summary>
        /// This method is used to fetch the PO invoice from Invoice module and store it in local machine.
        /// </summary>
        public async void ExportXl()
        {
            try
            {
                if (ProcessindentView != null)
                {
                    ...
                    await Task.Run(async () =>
                    {
                        var result = await _indentService.GetPOInvoiceFromInvoiceModule(ProcessindentView.PoNumber);
                        ...
                        if (result != null && result.IsSuccess)
                        {
                            Application.Current.Dispatcher.Invoke(() =>
                            {
                                try
                                {
                                    string url = result.Data.data.sign_s3_urls._A4;
                                    Process.Start(url);
                                }
        ...
        private async void submitPopup(IndentApprovalViewModel viewModel)
        {
            try
            {
                ...
                await Task.Run(async () =>
                {
                    ...
                    if (viewModel.AddSupplierToIndent.TotalAvailableQty > 0) {
                        if (viewModel.AddSupplierToIndent.PaymentType == "Partial")
                            partialPayment.Add(new AddPartialPayment() {
                                ...
                                PaymentTotal = (viewModel.AddSupplierToIndent.PartialPayment.PaymentTotal-viewModel.AddSupplierToIndent.PartialPayment.TdsAmount),
                                TdsAmount=viewModel.AddSupplierToIndent.PartialPayment.TdsAmount
                               
                            });
                        else
                            partialPayment.Add(new AddPartialPayment() { PaymentDate = AddSupplierToIndent.CreditPeriod == "0" ? string.Format("{0:dd-MM-yyyy}", DateTime.Now) : AddSupplierToIndent.PaymentDate, PaymentTotal = AddSupplierToIndent.PayableAmount,TdsAmount=AddSupplierToIndent.TdsDeductAmount });
                        ...
                        model.TransportChargesPayer = viewModel.TransportChargesPayer == null ? "" : viewModel.TransportChargesPayer;
                    ...
                    if (viewModel.DiscountDetails!=null  && DiscountDetails.Count > 0)
                    {
                       
                       foreach( var item in viewModel.DiscountDetails)
                        {
                            if (!string.IsNullOrEmpty(item.Name))
                            {
                                discountModels.Add(new DiscountModel()
                                {
                                    Name = item.Name,
                                    Percentage = item.DisocuntPercentage,
                                    Value = item.DisocuntValue,
                                });
                            }
                            
                        }
                    }
                    model.Discounts = discountModels;
                    model.TC=viewModel.TC==null?"":viewModel.TC;
                    model.TransportChargesPayer = viewModel.TransportChargesPayer == null ? "" : viewModel.TransportChargesPayer;
        ...
        public void EstimatePriceChangeCalculation(object obj)
        {
            ...
            foreach (var item in AddSupplierToIndent.Products)
            {
                ...
                if (item.SelectedGSTslab != null && AddSupplierToIndent.GSTType== "Exclusive GST")
                {
        ...
        public float EstimateCalculation(object obj)
        {
            try
            {
                ...
                {
                    if (item.SelectedGSTslab != null && item.SelectedGSTslab.GstValue!=0 && item.SelectedGSTslab.GstType=="Exclusive")
                    {
        ...
        public void SelectionChangePercent(object obj)
        {
            try
            {
                ...
                if (_viewModel != null && AddSupplierToIndent.PartialPayments == null && _viewModel.AddSupplierToIndent.PartialPayment.Percentage>0)
                {

                    if (AddSupplierToIndent.GSTType == "Inclusive GST")
                    {
                        var total = AddSupplierToIndent.PartialPayment.Percentage != null ? (float)Math.Round((((AddSupplierToIndent.PayableAmount + AddSupplierToIndent.TdsDeductAmount) * (float)AddSupplierToIndent.PartialPayment.Percentage) / 100), MidpointRounding.AwayFromZero) : 0;
                        ...
                        else
                        {
                            ...
                            AddSupplierToIndent.PartialPayment.Price = 0;
                            AddSupplierToIndent.PartialPayment.TdsAmount = 0;
                            // tds calculation 10 percent deduct
                            //TDSCalculationInitialPay();

                        }
                    }
                    else
                    {
                        var total = AddSupplierToIndent.PartialPayment.Percentage != null ? (float)Math.Round(((AddSupplierToIndent.NetPayableWithOutGst * (float)AddSupplierToIndent.PartialPayment.Percentage) / 100), MidpointRounding.AwayFromZero) : 0;
                        var paidAmount = AddSupplierToIndent.PartialPayments != null ? AddSupplierToIndent.PartialPayments.Sum(x => x.PaymentTotal) : 0;
                        if ((total + paidAmount) > AddSupplierToIndent.PayableAmount)
                        {
                            AddSupplierToIndent.PartialPayment.PaymentTotal = 0;
            ...
            try
            {
                ...
                if (_remainPay != null)
                {
                    ...
                    if (_remainPay.ProcessindentView.PaymentsList.Any() && _remainPay.ProcessindentView.PaymentsList != null)
                    {
                        ...
                        var balance = (_remainPay.ProcessindentView.PayableAmount - _remainPay.ProcessindentView.PaymentsList.Sum(x => x.PaymentTotal+x.TdsAmount));
                        ...
                        AddSupplierToIndent.GSTType =( listOfProduct.FirstOrDefault() != null && listOfProduct.FirstOrDefault().SelectedGSTslab?.GstType=="Exclusive")? "Exclusive GST" : "Inclusive GST";
                        ...
                        // Checks if Inventory detail is not null and sets IsSelected to false. 
                        if (AddSupplierToIndent != null && AddSupplierToIndent.InventoryDetail != null)
                        {
                            AddSupplierToIndent.InventoryDetail.IsSelected = false;
                        }
                        ...
                        //var _result = await _supplierService.GetSupplierTDSDetails((int)_remainPay.ProcessindentView.SupplierDetails.SupplierId);
                        //if (_result != null && _result.IsSuccess)
                        //{
                        //    if (_result.Data.IsTDSApplicable)
                        //    {
                        //        AddSupplierToIndent.IsTDSApplicable = _result.Data.IsTDSApplicable;
                        //        AddSupplierToIndent.TdsDeduct = _result.Data.TDSDeduct;
                        //        AddSupplierToIndent.TdsLimit = _result.Data.TDSLimit;
                        //        AddSupplierToIndent.PurchaseAmount = _result.Data.PurchasedAmount;
                        //        AddSupplierToIndent.CreditNoteAmount = _result.Data.CreditNoteAmount;
                        //    }
                        //    else if (((Math.Round( _result.Data.PurchasedAmount)-_result.Data.CreditNoteAmount) + AddSupplierToIndent.TotalAmount + AddSupplierToIndent.TransportCharges) > _result.Data.TDSLimit)
                        //    { // purchase amount less than tds limit calculation
                        //        AddSupplierToIndent.IsTDSApplicable = true;
                        //        AddSupplierToIndent.TdsDeduct = _result.Data.TDSDeduct;
                        //        AddSupplierToIndent.TdsLimit = _result.Data.TDSLimit;
                        //        AddSupplierToIndent.PurchaseAmount = _result.Data.PurchasedAmount;
                        //        AddSupplierToIndent.CreditNoteAmount = _result.Data.CreditNoteAmount;
            ...
            try
            {
                ...
                if (_viewModel != null)
                {
                    if (_viewModel.AddSupplierToIndent.AgainstType == "Against Partial")
                    {
                        ...
                        var _result = await _indentService.UpdatePartialPayment(_viewModel.ProcessindentView.Id, (_viewModel.AddSupplierToIndent.PartialPayment.PaymentTotal-_viewModel.AddSupplierToIndent.PartialPayment.TdsAmount), _viewModel.AddSupplierToIndent.PartialPayment.PaymentDate,_viewModel.AddSupplierToIndent.PartialPayment.TdsAmount);
                        if (_result != null && _result.IsSuccess)
                        {
                    ...
                    else
                    {
                        ...
                        var _result = await _indentService.UpdatePartialPayment(_viewModel.ProcessindentView.Id,( _viewModel.AddSupplierToIndent.PartialPayment.PaymentTotal), _viewModel.AddSupplierToIndent.PartialPayment.PaymentDate,_viewModel.AddSupplierToIndent.InvoiceAgainst.RemainTdsDeduct);
                        if (_result != null && _result.IsSuccess)
                        {
                            _notificationService.ShowMessage(_result.Data, Common.NotificationType.Success);
                            _eventAggregator.GetEvent<IntentNewTabCreateEvent>().Publish(model);
        ...
        public void PartialPriceChange(object obj)
        {
            ...
            {
                var _viewModel = (IndentApprovalViewModel)obj;
                if (_viewModel != null && AddSupplierToIndent.PartialPayments == null && _viewModel.AddSupplierToIndent.PartialPayment.Price>0)
                {
                    if (AddSupplierToIndent.GSTType == "Inclusive GST")
                    {
                        _viewModel.AddSupplierToIndent.PartialPayment.Percentage = null;
                        var _payAmount = AddSupplierToIndent.PayableAmount;
                        var _partialAmount = AddSupplierToIndent.PartialPayments != null ? AddSupplierToIndent.PartialPayments.Sum(x => x.PaymentTotal+x.TdsAmount) : 0;
        ...
        public async void AddPriceContinue()
        {
            try
            {
                if (AddSupplierToIndent != null && AddSupplierToIndent.Products.Count > 0)
                    ...
                    foreach (var item in AddSupplierToIndent.Products)
                    {
                        ...
                        if (item.AvailableQty > 0) {
                            ...
                            if (Math.Round(item.EstimatedPrice, 2) == 0) {
                                _notificationService.ShowMessage("Please enter landing price at row " + row, Common.NotificationType.Error);
                                return;
                            }
                ...
                AddSupplierToIndent.PartialPayment.TdsAmount = 0;
                TransportChargesPayer = null;
                //var _result = await _supplierService.GetSupplierTDSDetails((int)AddSupplierToIndent.SelectedSupplier.SupplierId);
                //if (_result != null && _result.IsSuccess)
                //{
                //    if (_result.Data.IsTDSApplicable)
                //    {
                //        AddSupplierToIndent.IsTDSApplicable = _result.Data.IsTDSApplicable;
                //        var _tdsAmount = ((AddSupplierToIndent.TotalAmount + AddSupplierToIndent.TransportCharges) / 100) * _result.Data.TDSDeduct;
                //        AddSupplierToIndent.TdsDeductAmount = (float) Math.Round( _tdsAmount,2);
                //        AddSupplierToIndent.PayableAmount = (AddSupplierToIndent.TotalAmount+AddSupplierToIndent.TransportCharges) - (float)Math.Round(_tdsAmount,2);
                //        AddSupplierToIndent.TdsDeduct = _result.Data.TDSDeduct;
                //        AddSupplierToIndent.TdsLimit = _result.Data.TDSLimit;
                //        AddSupplierToIndent.PurchaseAmount = _result.Data.PurchasedAmount;
                //        AddSupplierToIndent.CreditNoteAmount = _result.Data.CreditNoteAmount;
                ...
                //    { // purchase amount less than tds limit calculation
                //        AddSupplierToIndent.IsTDSApplicable = true;
                //        var _total = (AddSupplierToIndent.TotalAmount + AddSupplierToIndent.TransportCharges);
                //        var _purchase = Math.Round(_result.Data.PurchasedAmount) - _result.Data.CreditNoteAmount;
                //        var _tdsAmount = ((((_total+_purchase)-_result.Data.TDSLimit) / 100) * _result.Data.TDSDeduct);
                //        AddSupplierToIndent.TdsDeductAmount = (float)Math.Round(_tdsAmount,2);
                //        AddSupplierToIndent.PayableAmount = (AddSupplierToIndent.TotalAmount + AddSupplierToIndent.TransportCharges) - (float)Math.Round(_tdsAmount,2);
                //        AddSupplierToIndent.TdsDeduct = _result.Data.TDSDeduct;
                //        AddSupplierToIndent.TdsLimit = _result.Data.TDSLimit;
        ...
        public void SecondSelectionChangePercent(object obj)
        {
            try
            {
                ...
                if (_viewModel != null && AddSupplierToIndent.PartialPayments != null)
                {
                    ...
                    if (AddSupplierToIndent.GSTType == "Inclusive GST")
                    {
                        var total = AddSupplierToIndent.PartialPayment.Percentage != null ?(float)Math.Round( (((AddSupplierToIndent.PayableAmount) * (float)AddSupplierToIndent.PartialPayment.Percentage) / 100),MidpointRounding.AwayFromZero) : 0;
                        var paidAmount = AddSupplierToIndent.PartialPayments != null ? AddSupplierToIndent.PartialPayments.Sum(x => x.PaymentTotal+x.TdsAmount) : 0;
                        ...
                        if ((total + paidAmount) > (AddSupplierToIndent.PayableAmount))
                        {
                            ...
                            AddSupplierToIndent.PartialPayment.Percentage = null;
                            AddSupplierToIndent.PartialPayment.TdsAmount = 0;
                            //var withOuttdsPaidAmount = AddSupplierToIndent.PartialPayments != null ? AddSupplierToIndent.PartialPayments.Sum(x => x.PaymentTotal) : 0;

                            //if ((total + withOuttdsPaidAmount) > (AddSupplierToIndent.PayableAmount)){
                            //    AddSupplierToIndent.PartialPayment.PaymentTotal = 0;
                            //    AddSupplierToIndent.PartialPayment.Percentage = null;
                            //    //AddSupplierToIndent.PartialPayment.TdsAmount = 0;
                            //}
                            //else
                            //{
                            //    AddSupplierToIndent.PartialPayment.PaymentTotal = (total-(paidAmount-withOuttdsPaidAmount));
                            //    AddSupplierToIndent.PartialPayment.Price = 0;
                            //}
                        }
            ...
            try
            {
                ...
                if (view != null)
                {
                    ...
                    if (totalAmount < AddSupplierToIndent.PayableAmount)
                    {
                        ...
                        AddSupplierToIndent.InvoiceAgainst.TotalTdsDeducted = AddSupplierToIndent.PartialPayments.Sum(x => x.TdsAmount);
                        AddSupplierToIndent.InvoiceAgainst.BalancePay = AddSupplierToIndent.InvoiceAgainst.TotalInvoiceAmount - (AddSupplierToIndent.InvoiceAgainst.TotalPartiallyPaid + AddSupplierToIndent.InvoiceAgainst.TotalTdsDeducted);
                        //last invoice
                        if (AddSupplierToIndent.InvoiceAgainst.BalancePay < -1)
                        {
                            AddSupplierToIndent.InvoiceAgainst.BalancePay = (AddSupplierToIndent.PayableAmount - (AddSupplierToIndent.InvoiceAgainst.TotalPartiallyPaid + AddSupplierToIndent.InvoiceAgainst.TotalTdsDeducted));
                        }
                    }
            ...
            try
            {
                ...
                if (view != null)
                {
                    ...
                    else if (totalAmount == AddSupplierToIndent.PayableAmount)
                    {
                        ...
                        AddSupplierToIndent.InvoiceAgainst.RemainTdsDeduct = 0;
                        AddSupplierToIndent.InvoiceAgainst.TotalTdsDeducted = 0;
                    }
                    else if (totalAmount > AddSupplierToIndent.PayableAmount)
                    {
                        AddSupplierToIndent.InvoiceAgainst.TotalInvoiceAmount = 0;
                        ...
                        foreach (var item in IndentProducts)
                        {
                            ...
                            if (ReviewProducts.Where(x => x.ProductId == item.SelectedProduct.ProductId).FirstOrDefault() == null)
                            {
                                var _addProduct = new IndentViewProduct()
                                {
                                    ...
                                    RmQty = AppConstants.ROLE_REGIONAL_MANAGER == AppConstants.USER_ROLES[0]?item.Quantity:0,
                                    TmQty = AppConstants.ROLE_TERRITORY_MANAGER == AppConstants.USER_ROLES[0]?item.Quantity:0
        ...
        public void TDSCalculationInitialPay()
        {
            try
            {
                if (AddSupplierToIndent.IsTDSApplicable)
                {
                    //already purchased above 50lack 
                    if (Math.Round(AddSupplierToIndent.PurchaseAmount - AddSupplierToIndent.CreditNoteAmount) > AddSupplierToIndent.TdsLimit)
                    {
                        var _tdsAmount = (AddSupplierToIndent.PartialPayment.PaymentTotal / 100) * AddSupplierToIndent.TdsDeduct;
                        AddSupplierToIndent.PartialPayment.TdsAmount = (float)Math.Round(_tdsAmount,2);
                    }
                    else
                    {
                        if (((AddSupplierToIndent.PurchaseAmount - AddSupplierToIndent.CreditNoteAmount) + AddSupplierToIndent.PartialPayment.PaymentTotal) > AddSupplierToIndent.TdsLimit)
                        {
                            var _abovePurchaseAmount = (((AddSupplierToIndent.PurchaseAmount - AddSupplierToIndent.CreditNoteAmount) + AddSupplierToIndent.PartialPayment.PaymentTotal) - AddSupplierToIndent.TdsLimit);

                            //var _amount = AddSupplierToIndent.PartialPayment.Percentage != null ? (float)Math.Round((((_abovePurchaseAmount) * (float)AddSupplierToIndent.PartialPayment.Percentage) / 100), MidpointRounding.AwayFromZero) : 0;
                            var _tdsAmount = (_abovePurchaseAmount / 100) * AddSupplierToIndent.TdsDeduct;
                            AddSupplierToIndent.PartialPayment.TdsAmount = (float)Math.Round(_tdsAmount,2);
                        }
                    }
        ...
        public void TDSCalculationSecondPay()
        {
            try
            {
                if (AddSupplierToIndent.IsTDSApplicable)
                {
                    ...
                    {
                        if (((Math.Round( AddSupplierToIndent.PurchaseAmount) - AddSupplierToIndent.CreditNoteAmount) + AddSupplierToIndent.PartialPayments.Sum(x => x.TdsAmount + x.PaymentTotal)) > AddSupplierToIndent.TdsLimit)
                        {
                            var _tdsAmount = (AddSupplierToIndent.PartialPayment.PaymentTotal / 100) * AddSupplierToIndent.TdsDeduct;
                            AddSupplierToIndent.PartialPayment.TdsAmount = (float)Math.Round(_tdsAmount,2);
                        ...
                        else if (((Math.Round( AddSupplierToIndent.PurchaseAmount) - AddSupplierToIndent.CreditNoteAmount) + AddSupplierToIndent.PartialPayments.Sum(x => x.TdsAmount + x.PaymentTotal) + AddSupplierToIndent.PartialPayment.PaymentTotal) > AddSupplierToIndent.TdsLimit)
                        {
                            var _abovePurchaseAmount = (((Math.Round( AddSupplierToIndent.PurchaseAmount) - AddSupplierToIndent.CreditNoteAmount) + AddSupplierToIndent.PartialPayments.Sum(x => x.TdsAmount + x.PaymentTotal) + AddSupplierToIndent.PartialPayment.PaymentTotal) - AddSupplierToIndent.TdsLimit);
        ...
        public void TDSCalculationPriceInitial()
        {
            try
            {
                if (AddSupplierToIndent.IsTDSApplicable)
                {
                    ...
                    {
                        if (((Math.Round( AddSupplierToIndent.PurchaseAmount) - AddSupplierToIndent.CreditNoteAmount) + AddSupplierToIndent.PartialPayment.PaymentTotal) > AddSupplierToIndent.TdsLimit)
                        {
                            var _abovePurchaseAmount = (((Math.Round( AddSupplierToIndent.PurchaseAmount) - AddSupplierToIndent.CreditNoteAmount) + AddSupplierToIndent.PartialPayment.PaymentTotal) - AddSupplierToIndent.TdsLimit);
        ...
        public void TDSCalculationPriceSecond()
        {
            try
            {
                if (AddSupplierToIndent.IsTDSApplicable)
                {
                    ...
                    {
                        if (((Math.Round(AddSupplierToIndent.PurchaseAmount)- AddSupplierToIndent.CreditNoteAmount) + AddSupplierToIndent.PartialPayments.Sum(x=>x.PaymentTotal+x.TdsAmount)) > AddSupplierToIndent.TdsLimit)
                        {
        ...
        public void GSTChange(object obj)
        {
            try
            {
                if (AddSupplierToIndent.Products != null && AddSupplierToIndent.Products.Count > 0)
        ...
        private ObservableCollection<StepInfo> _stepsInfo;

        public ObservableCollection<StepInfo> StepsInfo {
            get => _stepsInfo;
            set => SetProperty(ref _stepsInfo, value);
        }
        ...
        private StepInfo _selectedItem;
        public StepInfo SelectedItem {
            get => _selectedItem;
            set => SetProperty(ref _selectedItem, value);
        }

        private void PayBackCommand(object obj) {
            try {
        ...
        public  void AddPaymentContinue()
        {
            try
            {
                if (AddSupplierToIndent.TotalAvailableQty > 0)
                ...
                DiscountDetails = new ObservableCollection<DiscountCardViewModel>();
                TC = null;
                var _discount = _container.Resolve<DiscountCardViewModel>();
                DiscountDetails.Add(_discount);
        ...
        private void AddNewDiscountField(object obj)
        {
            try
            {
                var model = obj as DiscountCardViewModel;
                if (model != null)
                {
                    if (DiscountDetails.Count() >= 7)
                    {
                        _notificationService.ShowMessage("Discount should not more than 7 fields", NotificationType.Error);
                        return;
                    }
                    DiscountDetails.Add(_container.Resolve<DiscountCardViewModel>());
        ...
        private void RemoveDiscountfield(object obj)
        {
            try
            {
                var model = obj as DiscountCardViewModel;
                if (model != null)
                {
                    if (DiscountDetails.Count() > 1)
                    {
                        var remove = DiscountDetails?.FirstOrDefault(x => x.DiscountCardGUIDId == model.DiscountCardGUIDId);
                        DiscountDetails.Remove(remove);
                    }
        ...
        public void DiscountValueCal(object obj)
        {
            try
            {

                var _model = obj as DiscountCardViewModel;
                if (_model != null)
                {
                    var _discount = DiscountDetails.FirstOrDefault(x => x.DiscountCardGUIDId == _model.DiscountCardGUIDId);
                    if (_discount != null && _model.DisocuntValue>0)
                    {
                        _discount.DisocuntPercentage = 0;
                    }
        ...
        public void DiscountPercentageCal(object obj)
        {
            try
            {
                ...
                {
                    var _discount = DiscountDetails.FirstOrDefault(x => x.DiscountCardGUIDId == _model.DiscountCardGUIDId);
                    if (_discount != null && _model.DisocuntPercentage>0)
                    {
                        _discount.DisocuntValue = 0;
                    }
        ...
        private ObservableCollection<DiscountCardViewModel> _discountDetails = new ObservableCollection<DiscountCardViewModel>();
        public ObservableCollection<DiscountCardViewModel> DiscountDetails
        {
            get { return _discountDetails; }
            set { SetProperty(ref _discountDetails, value); }
        }
        ...
        private List<string> _transportChargesPayers;

        public List<string> TransportChargesPayers
        {
            get { return  _transportChargesPayers; }
            set { SetProperty(ref _transportChargesPayers , value); }
        }

        private string  _transportChargespayer;

        public string  TransportChargesPayer
        {
            get { return _transportChargespayer; }
            set { SetProperty(ref _transportChargespayer, value); }
        }
...
//if (AddSupplierToIndent.IsTDSApplicable)
                        //{
                        //    if (((Math.Round(AddSupplierToIndent.PurchaseAmount)-AddSupplierToIndent.CreditNoteAmount)-AddSupplierToIndent.PayableAmount) > AddSupplierToIndent.TdsLimit)
                        //    {
                        //        var _tdsAmount = (AddSupplierToIndent.InvoiceAgainst.BalancePay / 100) * AddSupplierToIndent.TdsDeduct;
                        //        AddSupplierToIndent.InvoiceAgainst.RemainTdsDeduct = (float)Math.Round(_tdsAmount,2);
                        //        AddSupplierToIndent.InvoiceAgainst.BalancePay = AddSupplierToIndent.InvoiceAgainst.TotalInvoiceAmount - (AddSupplierToIndent.InvoiceAgainst.TotalPartiallyPaid + AddSupplierToIndent.InvoiceAgainst.TotalTdsDeducted + AddSupplierToIndent.InvoiceAgainst.RemainTdsDeduct);
                        ...
                        //            AddSupplierToIndent.InvoiceAgainst.RemainTdsDeduct = (float)Math.Round(_tdsAmount, 2);
                        //            AddSupplierToIndent.InvoiceAgainst.BalancePay = AddSupplierToIndent.InvoiceAgainst.TotalInvoiceAmount - (AddSupplierToIndent.InvoiceAgainst.TotalPartiallyPaid + AddSupplierToIndent.InvoiceAgainst.TotalTdsDeducted + AddSupplierToIndent.InvoiceAgainst.RemainTdsDeduct); 
                        //            //var _abovePurchaseAmount = ((Math.Round( AddSupplierToIndent.PurchaseAmount)-AddSupplierToIndent.CreditNoteAmount) - AddSupplierToIndent.TdsLimit);
                        //            //var _tdsAmount = (_abovePurchaseAmount / 100) * AddSupplierToIndent.TdsDeduct;
                        //            //AddSupplierToIndent.InvoiceAgainst.RemainTdsDeduct = (float)Math.Round((_tdsAmount - AddSupplierToIndent.InvoiceAgainst.TotalTdsDeducted),2);
                        //        }
                        //    }
