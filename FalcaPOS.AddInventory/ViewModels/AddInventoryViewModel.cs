...
, IInvoiceFileService invoiceFileService, IIndentService indentService, IDialogService dialogService)
        {
            ...
            _eventAggregator.GetEvent<SignalRIndentStatusToInTransiteEvent>().Subscribe(RefreshIndentLoadData, ThreadOption.PublisherThread);

            _eventAggregator.GetEvent<IndentStatusChangeToSahayaPendingEvent>().Subscribe(RefreshIndentLoadDataDueToSahaya, ThreadOption.PublisherThread);
        ...
        private async Task AddNewStockProduct()
        {
            try
            {
                ...
                _logger.LogInformation("Adding new stock products");
        ...
        private AddStockProductModel GetInvoiceVM()
        {
            try
            {
               ...
               // _vm.InvoiceNumber = InvoiceNumber;
                ...
                _vm.DefectiveQty = _vm.StockProducts.Where(x => x.DefectiveQty > 0).Count();
                if (InvoiceDiscountFlat > 0)
                {
            ...
            {
                _logger.LogError("Error in get invoice vm", _ex);
                throw;
            }
        ...
        private List<StockProductModel> GetStockProducts()
        {
            ...
            {
                _logger.LogError("Error in get stock product method", _ex);
                throw;
            }
        }
        ...
        private StockProductModel GetProduct(ProductCardViewModel product)
        {
            try
            {
                ...
                return new StockProductModel
                {
                    ...
                    DefectiveQty=product.DefectiveQty,
        ...
        private bool ValidateProductInfo(ProductCardViewModel product, int index = 0)
        {
            ...
            if (product != null)
            {
                if (product.IsGroupTrackMode)
                {
                    ...
                    {
                        _notificationService.ShowMessage($"Enter valid received qty for product {product.SelectedProduct.Name}" +
                            $" at row {index + 1}", NotificationType.Error);
                        return false;
                ...
                if (product.DateOfManufacture.Value.Date>DateTime.Now.Date)
                {
                    _notificationService.ShowMessage($"Manufacture date should not greater then current date at row {index + 1}", NotificationType.Error);
        ...
        private bool ValidateData()
        {
            ...
            if (IsDcNumber)
            {
                ...
                if (!string.IsNullOrEmpty(DcNumber))
                {
                    if (!DcNumber.Any(x => char.IsLetterOrDigit(x)))
                    {
                        _notificationService.ShowMessage("Invoice number cannot contain only special characters", NotificationType.Error);
                        return false;
                    }
                ...
                if (DcNumberDate.Value.Date > DateTime.Now.Date)
                {
                    _notificationService.ShowMessage("DC Date should not greater then today date", NotificationType.Error);
                    return false;
                }
            ...
            else
            {
                ...
                {
                    if (!InvoiceNumber.Any(x => char.IsLetterOrDigit(x)))
                    {
                        _notificationService.ShowMessage("should not allow only special characters as invoice number", NotificationType.Error);
                        return false;
                    }
                ...
                if (InvoiceDate.Value.Date > DateTime.Now.Date)
                {
                    _notificationService.ShowMessage("Invoice Date should not greater then today date", NotificationType.Error);
                    return false;
                }
            }
        ...
        public void RefreshIndentLoadData(IndentViewModel indentView)
        {
            try
            {
                //don't refresh if they are in middle 
                if (ProductCards.Count() == 0)
                {
                    LoadIndent();
                }
        ...
        // Refreshs the indent list in Add inventory page once a PO request is raised from Sahaya.
        public void RefreshIndentLoadDataDueToSahaya()
        {
            try
            {
                //don't refresh if they are in middle 
                if (ProductCards.Count() == 0 || SelectedIndent.Status == "sahayapending")
                {
                    LoadIndent();
                }
            }
