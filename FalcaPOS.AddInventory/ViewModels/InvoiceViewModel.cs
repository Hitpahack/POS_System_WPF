...
namespace FalcaPOS.AddInventory.ViewModels
{
    public class InvoiceViewModel : BindableBase
    {
        ...
        public async void Storechange(object obj)
        {
            try
            {
                ...
                if (value != null && value.Name != "All")
                {
                    ...
                    InvoiceList = new ObservableCollection<InvoiceListViewModel>(InvoiceList?.Where(x => x.Store == value.Name).OrderBy(x => x.Store).ToList());
                }
                else
                {
                    await LoadInvoices();
        ...
        private bool ValidateInvoice(InvoiceModel invoice)
        {
            ...
            {
                _notificationService.ShowMessage("Invoice should have at least one product", Common.NotificationType.Error);
                return false;
            }
            ...
            foreach (var _product in invoice.StockProducts)
            {
                ...
                //Removed Check for selling price. (Bug: SUGI 735).
                ...
                {
                    if (_product.ProductMRP <= 0) {
                        _notificationService.ShowMessage($"Please add MRP for fertilizers",Common.NotificationType.Error);

                        return false;

                    }

                    // base on bug sugi-2044 commanted this validation
                ...
                if (_product.ProductSellingPrice == 0)
                {
                        ...
                        _notificationService.ShowMessage("Please enter valid selling price",Common.NotificationType.Error);
                        return false;
                    
                }

                // Checks if margin obtained for a product is less than 0 and throws error.
                if (_product.Margin < 0)
                {
                    _notificationService.ShowMessage("Margin cannot be negative", Common.NotificationType.Error);
                    return false;
                }
        ...
        private InvoiceProductCardViewModel GetProduct(StockProductViewModel product, float discountPercent, float discountFlat, int productTotalCount, string applyDiscounttype, string state)
        {
            ...
            _product.LandingRate = product.LandingRate;
