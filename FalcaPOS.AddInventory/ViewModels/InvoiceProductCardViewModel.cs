...
namespace FalcaPOS.AddInventory.ViewModels
{
    public class InvoiceProductCardViewModel : BindableBase
    {
        ...
        private float _landingRate;
        public float LandingRate
        {
            get { return _landingRate; }
            set { SetProperty(ref _landingRate, value); }
        }
        ...
        private void CalculateMargin()
        {
            if (InvoiceProductDiscount > 0)
            {
                // Discounted rate = Rate + GST - Discount
            ...
            //// Calculate Margin based on landing rate even though discount is added or not for the product.
            //Margin = (float)(ActualPrice - LandingRate - Misc);
            //MarginPercentage = (Margin / ProductSellingPrice) * 100;

        }

        private void calculateActulPrice()
        {
            // Without GST price from Selling Price
