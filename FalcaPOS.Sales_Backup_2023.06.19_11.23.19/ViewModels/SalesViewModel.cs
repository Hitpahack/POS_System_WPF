...
namespace FalcaPOS.Sales.ViewModels
{
    public class SalesViewModel : BindableBase
    {
            ...
            try
            {
                if (model != null)
                {
                        ...
                        if (_SourceModel != null)
                        {
                            ...
                            {
                                _notificationService.ShowMessage("Customer details updated failed", NotificationType.Error);
                                return;
                            }
                        }
        ...
        private async void LoadInvoiceNumber()
        {
            ...
            if (_result != null && _result.IsSuccess)
            {
                SalesInvoiceNumber = _result.Data;
            }
        ...
        private async void SaveSalesDetails()
        {
            ...
            await Task.Run(async () =>
            {
                var result = await _salesService.AddSales(_salesDetails);
        ...
        private void RemoveProduct(object obj)
        {
            ...
            ProductCount = (SalesProducts != null && SalesProducts.Count() > 0) ? SalesProducts.Count() : 0;
        ...
        private void CalculateTotal()
        {
            try
            {
                ...
                if (SalesProducts != null && SalesProducts.Any())
                {
                    ...
                    PayableAmount = Math.Round((GrossTotal) + (float)TotalServiceCharge, 2, MidpointRounding.AwayFromZero);
        ...
        public void SaveCustomer(object obj)
        {
            try
            {
                ...
                {
                    _notificationService.ShowMessage("Please enter farmer name", NotificationType.Error);
                    return;
                }
                if (string.IsNullOrEmpty(SCustomerType))
                {
                    _notificationService.ShowMessage("Please select customer type", NotificationType.Error);
                    return;
                }
                if (SState == null)
                {
