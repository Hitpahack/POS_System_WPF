...
namespace FalcaPOS.Denomination.ViewModel
{
    ...
    {
        /// <summary>
        /// Handles the button click command on the Submit button present in the Denomination Dialog.
        /// </summary>
        ...
        /// <summary>
        /// Handles the button click command for adding the Denomination Count in EOD Menu. 
        /// </summary>
        public DelegateCommand<object> AddDenominationCountCommand { get; private set; }
        ...
        public DenominationViewModel(Logger logger, IEventAggregator eventAggregator, IDenominationService denominationService, IDialogService dialogService, INotificationService notificationService, ProgressService progressService)
        {
            ...
            AddDenominationCountCommand = new DelegateCommand<object>(AddDenominationCountPopup);
        ...
        /// <summary>
        /// Converts the current date in the a given format. Ex: '13th Jul'.
        /// </summary>
        /// <param name="currentDate">Current Date.</param>
        /// <returns>Returns current date in a given format.</returns>
        private void FomatDateToGivenFormat(string denominationDate)
        {
            // If Denomination Date after verification of previous day's denomination is null, set the date to today's date.
            if (denominationDate == null)
            {
                currentDate = DateTime.Today;
            }
            else
            {
                currentDate = DateTime.ParseExact(denominationDate, "dd-MM-yyyy", null);
            }
            ...
            // Format the date in 'ddth MM' format
            FormattedCurrentDate = string.Format("{0}{1} {2}", day, suffix, month);

        }

        /// <summary>
        /// This method opens the pop up to add the count of all the denominations for the EOD cash.
        /// </summary>
        /// <param name="obj">object</param>
        private async void AddDenominationCountPopup(object obj)
        {
            try
            {
                AddDenominationCountView addDenominationCountView = new AddDenominationCountView();
                addDenominationCountView.DataContext = this;
                await DialogHost.Show(addDenominationCountView, "RootDialog", AddDenominationCountEventHandler);
        ...
        private void ResetAllDenominationData()
        {
            try
            {
        ...
        /// <summary>
        /// Handles the event when user clicks on Submit or Cancel in the Denomination Dialog.
        /// </summary>
        /// <param name="sender">Object.</param>
        /// <param name="eventArgs">EventArgs.</param>
        private async void AddDenominationCountEventHandler(object sender, DialogClosingEventArgs eventArgs)
        {
            try
            {
        ...
        private void SalesReturnCashCal(object param)
        {
            try
            {
                ...
                else
                {
                    ...
                    DenominationModel.ClosingCash = DenominationModel.AvailableCash - DenominationModel.Deposit;
                }
            }
            catch (Exception _ex)
            {
        ...
        public async void PreviousdayDenominationVerify()
        {
            try
            {
                ...
                if (_result != null && _result.IsSuccess)
                {
                    ...
                    if (_result.Data.Data.ToString() ==  "New store")
                    {
                        IsNewStore = true;
                    }
                    else
                ...
                else
                {
                    ...
                    StartDate = Convert.ToDateTime(DenominationDate);
                    EndDate = Convert.ToDateTime(DenominationDate);
                ...
                FomatDateToGivenFormat(DenominationDate);
            }
            catch (Exception _ex)
            {
                _logger.LogError(_ex.Message);
        ...
        public void AddDeposit(object obj)
        {
            try
            {
                ...
                // If Deposit amount is not less than or equal to closing cash, throw an error.
                if (! (DepositModel.DepostAmount <= DenominationModel.ClosingCash))
                {
                    _notificationService.ShowMessage("Deposit amount is greater than closing cash", NotificationType.Error);
                    return;
                }
        ...
        private bool _isNewStore;

        public bool IsNewStore
        {
            get => _isNewStore;
            set => SetProperty(ref _isNewStore, value);
        }
        ...
        /// <summary>
        /// Stores the current date in a given format.
        /// </summary>
        private string _formattedCurrentDate;

        /// <summary>
        /// Gets and sets th current date in a given format.
        /// </summary>
        public string FormattedCurrentDate
        {
            get => _formattedCurrentDate;
            set => SetProperty(ref _formattedCurrentDate, value);
        }
