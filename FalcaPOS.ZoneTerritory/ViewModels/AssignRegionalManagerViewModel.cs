        ...
        private async void AddRegionalManagerCommandEvent(Object payload)
        {
            try
            {
                ...
                if (payload != null)
                {
                    ...
                    {
                        var _zoneId = (int)_payloadStoreID.Item2;
                        if (_zoneId > 0)
                        {
                            await _progressService.StartProgressAsync();
                            var _result = await _zoneService.AssignRegionalManagerToZone(_zoneId, SelectedRM.UserId, new System.Threading.CancellationToken());
