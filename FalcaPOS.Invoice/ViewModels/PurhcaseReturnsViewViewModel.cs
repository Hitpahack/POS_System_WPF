...
namespace FalcaPOS.Invoice.ViewModels
{
    public class PurhcaseReturnsViewViewModel : BindableBase
    {
        ...
        public DelegateCommand<object>  PurchaseReturnSearchCommand { get; set; }
        ...
        public PurhcaseReturnsViewViewModel(IStoreService storeService, ISupplierService supplierService, IInvoiceFileService invoiceFileService, IPurchaseInvoiceService purchaseInvoiceService, Logger logger, INotificationService notificationService, ProgressService ProgressService)
        {
           ...
           // SupplierSelectionChangeCommand = new DelegateCommand<object>(SelectionChange);
            ...
            StoreVisibility = (AppConstants.USER_ROLES[0] == AppConstants.ROLE_PURCHASE_MANAGER || AppConstants.USER_ROLES[0] == AppConstants.ROLE_FINANCE || AppConstants.USER_ROLES[0]== AppConstants.ROLE_CONTROL_MANAGER) ? true : false;

            ReturnStatus = AppConstants.PURCHASE_RETURN_STATUS;

            RefreshCommand = new DelegateCommand(Refresh);

            PurchaseReturnSearchCommand = new DelegateCommand<object>(LoadData);
        }
        ...
        public async void LoadData(object obj)
        {
            try
            {
                ...
                if (SelectedStore == null && AppConstants.USER_ROLES[0] != AppConstants.ROLE_BACKEND)
                {
                ...
                var status = SelectedStatus == "Return Requested" ? "Pre-C/N" : SelectedStatus == "Return Approved" ? "Pre-C/N-Approved" : SelectedStatus == "C/N Received" ? "Post-C/N" : SelectedStatus == "C/N Approved" ? "Created" : SelectedStatus == "C/N Rejected" ? "SendBack" : SelectedStatus == "C/N Adjusted" ? "Adjusted" : SelectedStatus == "C/N Partially Adjusted" ? "Partially Adjusted" : "";
                    ...
                    await Task.Run(async () =>
                    {
                        ...
                        Application.Current?.Dispatcher?.Invoke(() =>
                        {
                            if (_result != null && _result.IsSuccess)
                            {
                                ...
                                {
                                    var _isEnableBtn = (AppConstants.USER_ROLES[0] == AppConstants.ROLE_FINANCE ? true : false);
                                    var _isEnableEdit = (AppConstants.USER_ROLES[0] == AppConstants.ROLE_PURCHASE_MANAGER ? true : false);
                                    storeReturnModels.Add(new StoreReturnModel()
                                    {
                                        Id = item.Id,
                                        IsSelected = item.IsSelected,
                                        ...
                                        PurhcaseReturnProduct = item.PurhcaseReturnProduct,
                                        CreatedDatetime = item.CreatedDatetime,
                                        IsEnableApproveEditBtn = (_isEnableEdit == true && item.Status == "Pre-C/N" && item.IsApproved == false) ? true : false,
                                        IsEnableApproveSaveBtn = false,
                                        IsEnablePostApproveBtn = (_isEnableBtn == true && item.Status == "Post-C/N" && item.IsApproved == false) ? true : false,
        ...
        public void SaveProduct(object obj)
        {
            try
            {
                ...
                foreach (var item in viewModel.PurhcaseReturnProduct)
                {
                    ...
                    {
                        _notificationService.ShowMessage($"Please enter maximum available({item.RemainingStock}) stock qty to return", Common.NotificationType.Error);
                        return;
                    }
                }
                ...
                if (_viewModel != null)
                {
                    ...
                    await Task.Run(async () =>
                    {
                        ...
                        Application.Current?.Dispatcher?.Invoke(async () =>
                        {
                            if (_result != null && _result.IsSuccess && _result.Data != null)
                            {
                                _notificationService.ShowMessage("SendBack successfully", Common.NotificationType.Success);
        ...
        {
            get => _storeVisibility; 
            set => SetProperty(ref _storeVisibility, value); 
        }

        private List<string> _returnStatus;

        public List<string> ReturnStatus {
            get => _returnStatus;
            set => SetProperty(ref _returnStatus, value);
        }
